...
fetch('/api/createmarca')
            .then(response => response.json())
            .then(marcas => {
                const tableBody = document.getElementById('tablamarca').getElementsByTagName('tbody')[0];
                marcas.forEach(marca => {
                    const row = tableBody.insertRow();
                    row.insertCell(0).textContent = marca.codigomarca;
                    row.insertCell(1).textContent = marca.codigoproveedor;
                });
            })
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const messageContainer = form.querySelector('.message');
     ...
     table.addEventListener('click', function(event) {
        ...
        if (target && target.nodeName === "TR") { // Hemos encontrado la fila en la que se hizo clic
            ...
            form.codigomarca.value = cells[0].textContent;
            form.codigoproveedor.value = cells[1].textContent;
 ...
 document.getElementById('btneditar').addEventListener('click', function() {
    ...
    fetch(`/api/createmarca/${marcaData.codigomarca}`, {
        method: 'PUT',
        headers: {
    ...
    .then(data => {
        ...
        form.querySelector('.message').textContent = 'Marca actualizada con éxito.';
    ...
    .catch(error => {
        ...
        form.querySelector('.message').textContent = 'Error al actualizar la marca.';
    });
});

//evento de clic para el boton de eliminar
document.getElementById('btneliminar').addEventListener('click', function() {
    const codigomarca = form.codigomarca.value;
    ...
    if (confirm('¿Estás seguro de que deseas eliminar la marca?')) {
        fetch(`/api/createmarca/${codigomarca}`, { method: 'DELETE' })
        .then(response => response.json())
        .then(data => {
            //Mostrar mensaje de exito o manejar errores
            form.querySelector('.message').textContent = 'Marca eliminado con éxito.';
        ...
        .catch(error => {
            console.error('Error:', error);
            form.querySelector('.message').textContent = 'Error al eliminar la marca.';
        });
    }
});
