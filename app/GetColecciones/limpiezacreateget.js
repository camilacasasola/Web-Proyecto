...
fetch('/api/createlimpieza')
            .then(response => response.json())
            .then(limpiezas => {
                const tableBody = document.getElementById('tablalimpieza').getElementsByTagName('tbody')[0];
                limpiezas.forEach(limpieza => {
                    const row = tableBody.insertRow();
                    row.insertCell(0).textContent = limpieza.codigolimpieza;
                    row.insertCell(1).textContent = limpieza.descripcionlimpieza;
                    row.insertCell(2).textContent = limpieza.codigorestaurante;
                    row.insertCell(3).textContent = limpieza.cantidadlimpieza;
                });
            })
...
document.addEventListener('DOMContentLoaded', () => {
    const table = document.getElementById('tablalimpieza');
    ...
    const messageContainer = form.querySelector('.message');
     ...
     table.addEventListener('click', function(event) {
        ...
        if (target && target.nodeName === "TR") { // Hemos encontrado la fila en la que se hizo clic
            ...
            form.codigolimpieza.value = cells[0].textContent;
            form.descripcionlimpieza.value = cells[1].textContent;
            form.codigorestaurante.value = cells[2].textContent;
            form.cantidadlimpieza.value = cells[3].textContent;
    ...
    document.getElementById('btneditar').addEventListener('click', function() {
        const limpiezaData = {
            codigolimpieza: form.codigolimpieza.value,
            descripcionlimpieza: form.descripcionlimpieza.value,
            codigorestaurante: form.codigorestaurante.value,
            cantidadlimpieza: form.cantidadlimpieza.value
        };
        ...
        fetch(`/api/createlimpieza/${limpiezaData.codigolimpieza}`, {
            method: 'PUT',
            headers: {
        ...
        .then(data => {
            ...
            form.querySelector('.message').textContent = 'Limpieza actualizado con éxito.';
        ...
        .catch(error => {
            ...
            form.querySelector('.message').textContent = 'Error al actualizar el Limpieza.';
        });
    });

    //evento de clic para el boton de eliminar
    document.getElementById('btneliminar').addEventListener('click', function() {
        const codigolimpieza = form.codigolimpieza.value;

        //se creo una confirmacion para mas seguridad y que no se elimine la informacion inmediatamente
        if (confirm('¿Estás seguro de que deseas eliminar la Limpieza?')) {
            fetch(`/api/createlimpieza/${codigolimpieza}`, { method: 'DELETE' })
            .then(response => response.json())
            .then(data => {
                //Mostrar mensaje de exito o manejar errores
                form.querySelector('.message').textContent = 'Limpieza eliminada con éxito.';
            ...
            .catch(error => {
                console.error('Error:', error);
                form.querySelector('.message').textContent = 'Error al eliminar la Limpieza.';
            });
        }
    });
});