const ReservaB = require('../models/reservaBcreate');

exports.createReservaB = async (req, res) => {
    try {
      const nuevaReserva = new ReservaB({
      ...
      res.send(`<script>alert('${mensaje}'); window.location.href = '/ReservaB.html';</script>`);
    } catch (error) {
  ...
  exports.getReservas = async (req, res) => {
    try {
      ...
      const reservas = await ReservaB.find(query).sort({ fecha: 1, hora: 1 }); // Ordena las reservas por fecha y hora
