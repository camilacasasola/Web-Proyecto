...
const ComestibleSchema = new mongoose.Schema({
  //cada uno de los inputs del html se utilizan aca y llevan el mismo atributo 'name' para que concuerde con el html
  codigocomestible: { type: String, required: true, unique: true },
  descripcioncomestible: { type: String, required: true },
  cantidadcomestible: { type: String, required: true },
  codigorestaurante: { type: String, required: true },
  tipocomestible: { type: String, required: true },
});
...
module.exports = mongoose.model('comestiblecreate', ComestibleSchema);